...
75C and 80%.
''')
parser.add_argument('--temp', nargs='+', default=[30, 45, 55, 70], type=float, help='The temperature ranges where the fan speed will increase linearly')
parser.add_argument('--speed', nargs='+', default=[30, 50, 66, 99], type=float, help='The fan speed ranges')
...
parser.add_argument('--kill', default=True, help='Whether to kill existing Xorg sessions')  # 默认强制关闭X server
...
def log_output(command, ok=(0, )):
    ...
    try:
        p.wait(60)  # 如果子进程超过时长却没有执行结束, 将会抛出异常
        ...
        if p.returncode not in ok:
            print('\n'.join(output))
            raise ValueError('指令运行出错, 返回值 ' + str(p.returncode) + ': ' + ' '.join(command))
        return '\n'.join(output)
...
def manage_fans(displays):
    ...
    X servers"""
    try:
        speeds = {b: 0 for b in displays}  # 字典, 键是总线, 值是对应的风扇速度
        while True:
            for bus, display in displays.items():
                ...
                if s != speeds[bus]:
                    ...
                    set_speed(display, s)  # 该部分会报错This process is currently running setuid or setgid. This is not a supported use of GTK+. You must create a helper program instead. For further details, see: http://www.gtk.org/setuid.html
                    speeds[bus] = s
                else:
